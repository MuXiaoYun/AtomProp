...
class MultiHeadEdgeAttention_ParallelBetweenBondtypes(nn.Module):
    """
    Multi-head edge attention with bond-type-specific bilinear interaction.
    
    For each bond type t, learns a separate bilinear matrix A_t ∈ R^(H × D × D),
    and computes attention score for edge e of type t as:
        score_e = src_e^T @ A_t @ dst_e   (per head)
    ...
    def forward(
        ...
        """
        Compute edge-wise attention scores using bond-type-specific bilinear forms.
